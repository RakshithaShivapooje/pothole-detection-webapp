...
model = YOLO(r"C:\Users\sange\Downloads\best.pt")  # Adjust the path as needed
...
def register(location, highway_type, size, position):
    data = {"location": location, "highway_type": highway_type, "size": size, "position": position}
    send_email(data, 'mallik.shivapooje@gmail.com')  
...
def get_fallback_location():
    ...
    if g.latlng:
        ...
        st.error("Could not retrieve location information.")
...
def get_pothole_info():
    ...
    # Display the map using the location
